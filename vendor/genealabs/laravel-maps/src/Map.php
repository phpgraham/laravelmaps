...
class Map
{
    ...
    //context menu
    //TODO - add more features to menu items (such as what's here, search nearby, directions to and from here etc) and add design of menu options
    public $contextMenus = array();              //Array used to store ContextMenu information for output
    public $context_menu = false;                // Whether or not the context menu will be used
    public $contextMenuItems = array();          // An array of menu items to display, current options are (centre map, add marker here)
    ...
    //context menu
    public function add_context_menu($params = array())
    {
        $context = array();
        $context_output = '';
        $menu_items = '';
        ...
        if ($context['centerMap'] != "") {
            //
            $menu_items .= '<a id=\"menu0\" href=\"#\" onclick=\"panTo(clickedLatLng);\"><div class=\"context\">Center Map Here<\/div><\/a>';
        }
        if ($context['addMarker'] != "") {
            $menu_items .= '<a id=\"menu1\" href=\"#\" onclick=\"addMarker(clickedLatLng);\"><div class=\"context\">Add Marker Here<\/div><\/a>';
            //TODO - show below link when markers added, hide when no markers
            $menu_items .= '<a id=\"menu99\" href=\"#\" onclick=\"clearMarkers();\"><div class=\"context\">Clear Markers<\/div><\/a>';
        }


        //if()
        //{
            //$context_output .= 'google.maps.event.addListener('.$this->map_name.', "rightclick",function(event){showContextMenu(event.latLng);});';
            $context_output .= 'google.maps.event.addListener('.$this->map_name.', "rightclick", function(event) { mapRightClick(event.latLng) });
            ';

            $context_output .= 'function mapRightClick(currentLatLng) {
                clickedLatLng = currentLatLng;
                showContextMenu(currentLatLng);
            };
            ';

            // Hide context menu on several events
            $context_output .= 'google.maps.event.addListener('.$this->map_name.', "click", function(){
                hideContextMenu();
            });
            ';

            $context_output .= 'function showContextMenu(currentLatLng) {
                 var projection;
                 var contextmenuDir;
                 projection = '.$this->map_name.'.getProjection() ;
                 $(".contextmenu").remove();
                  contextmenuDir = document.createElement("div");
                   contextmenuDir.className  = "contextmenu";
                   contextmenuDir.id  = "contextmenu";
                   contextmenuDir.innerHTML = "'.$menu_items.'";
                 $('.$this->map_name.'.getDiv()).append(contextmenuDir);
                 setMenuXY(currentLatLng);
                 contextmenuDir.style.visibility = "visible";
            };
            ';

            $context_output .= 'function getCanvasXY(currentLatLng){
                var scale = Math.pow(2, '.$this->map_name.'.getZoom());
                var nw = new google.maps.LatLng(
                  '.$this->map_name.'.getBounds().getNorthEast().lat(),
                  '.$this->map_name.'.getBounds().getSouthWest().lng()
                );
                var worldCoordinateNW = '.$this->map_name.'.getProjection().fromLatLngToPoint(nw);
                var worldCoordinate = '.$this->map_name.'.getProjection().fromLatLngToPoint(currentLatLng);
            ...
            $context_output .= 'function setMenuXY(currentLatLng){
                var mapWidth = $("#'.$this->map_div_id.'").width();
                var mapHeight = $("#'.$this->map_div_id.'").height();
        ...
        array_push($this->contextMenus, $context_output);
    ...
    public function create_map()
    {
        ...
        if ($this->context_menu) {
            $this->output_js_contents .= 'var contextMenus;
            ';
        }
        ...
        // add contextMenu
        if(count($this->contextMenus)) {
            foreach ($this->contextMenus as $contextMenu) {
                $this->output_js_contents .= $contextMenu;
            }
        }
        ...
        if(count($this->contextMenus)) {
            //centre map here
            $this->output_js_contents .= 'function panTo(clickedLatLng) {
                '.$this->map_name.'.panTo(clickedLatLng);
                hideContextMenu();
            };
            ';

            //add marker
            $this->output_js_contents .= 'function addMarker(clickedLatLng) {
                var markerOptions = {
                    map: '.$this->map_name.',
            ...
            //clear markers
            $this->output_js_contents .= 'function clearMarkers() {
                for (var i = 0; i < markers_'.$this->map_name.'.length; i++) {
                    markers_'.$this->map_name.'[i].setMap(null);
                }
                lat_longs_'.$this->map_name.' = [];
                markers_'.$this->map_name.' = [];
            ...
            //hide context menu
            $this->output_js_contents .= 'function hideContextMenu() {
              $(".contextmenu").remove();
            };
            ';
